...
public class XMLJUnitResultFormatter implements JUnitResultFormatter, XMLConstants {
    ...
    public void endTest(Test test) {
        ...
        if (!failedTests.containsKey(test)) {
            ...
            String m = JUnitVersionHelper.getTestCaseClassName(test);
            ...
            // ------
            //若TestSuite抛错或者BeforeClass出错，则跳过Description的获取
            //此时无法获取当前的test case name！
            if (n != null && !m.equals("junit.framework.TestSuite") &&
                    !n.equals("initializationError")) {
                Method method = null;
                try {
                    Class<?> cls = Class.forName(m);
                    method = cls.getMethod(n, new Class[0]);
                } catch (ClassNotFoundException e) {
                    e.printStackTrace();
                } catch (NoSuchMethodException e) {
                    e.printStackTrace();
                }
                boolean hasDescription = method.isAnnotationPresent(Description.class);
                if (hasDescription) {
                    Description defectAnnotation = (Description) method.
                            getAnnotation(Description.class);
                    currentTest.setAttribute("description", defectAnnotation.value());
                }
            }
            // -------
